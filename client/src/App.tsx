...
import {Weather} from '../../api/types/WeatherResponse'
...
function App() {
  ...
  const [selectedWeather, setSelectedWeather] = useState<Weather | undefined>(undefined)

  /**
   * Fetch weather data for the postcode state
   */
  const getWeather = () => {
    getWeatherByPostcode(postcode).then((data:any) => {
  ...
  const onKeyUp = (e:any) => { //when the user presses enter, trigger a new API call
    if (e.key === "Enter") {
  ...
  const onChange = (e:any) => { //when the input changes
    setPostcode(e.target.value)
  }

  const onButtonClick = () => { //when the button is clicked
    setEditing(true)
  }
      ...
      <Container>
        ...
        <RightPanel>
          <div style={{
            ...
            {/* {weather && <> //todo
              <InfoLabel><div>PRECIPITATION</div><div style={{ fontWeight: 300}}>0%</div></InfoLabel>
              <InfoLabel><div>HUMIDITY</div><div style={{ fontWeight: 300}}>0%</div></InfoLabel>
              <InfoLabel><div>WIND</div><div style={{ fontWeight: 300}}>0%</div></InfoLabel>
            </>} */}
          </div>
          <WeekList>
            {weather?.map((weather:Weather, index:number) => {
              ...
              return <WeatherItem key={index} weather={weather} active={weather.date === selectedWeather?.date} onClick={onClick}/>
            })}
          </WeekList>
          {editing ? <Input name="input" value={postcode} onKeyUp={onKeyUp} onChange={onChange}></Input>:
